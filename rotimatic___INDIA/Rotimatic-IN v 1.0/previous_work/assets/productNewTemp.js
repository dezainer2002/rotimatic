...
const __IP__                      =   `https://api.ipregistry.co/?key=7lqx82u2hzc3mgqa&fields=location.country.name,location.city`;
...
if ( typeof pdpCheckRedirect !== 'undefined' && pdpCheckRedirect ) {
  getUserCountry();
}
...
async function getUserCountry() {
  try {
    ...
    const countries_              =   ["United States","Canada","Denmark","Norway","Sweden","Netherlands","France","Germany","United Arab Emirates","Saudi Arabia","Bahrain","Kuwait","Oman","Qatar","Singapore","United Kingdom","Ireland","Hong Kong","Australia","New Zealand","India","Pakistan"];
...
function isIndianCurrency() {
  try {

    const isTaxIncludedEle        =   $( `._taxIncluded_` ).length;

    if ( isTaxIncludedEle > 0 ) {

      const priceEleWidth         =   $( `.prodInfo ._price_` ).width();
      $( `._taxIncluded_` ).css( 'width', `${ priceEleWidth }px` );

    }

  }
  catch ( err ) {
    console.log( `ERROR isIndianCurrency`, err.message );
  }
}
...
.ready(function() {
  ...
  const selectedVariantAvailableQty       =   $( `#shopify-section-product-variantTabs ._variants_ ._item_.selected` ).attr( `available-qty` ) * 1;

  if ( typeof selectedVariantAvailableQty !== 'undefined' && selectedVariantAvailableQty == 0 ) {

    $( `#shopify-section-product-paymentBtn .addToCartForm .btn_add2Cart` )
    .prop( `disabled`, true )
    .addClass( `disabled` )
    .find( `> span` )
    .text( `SOLD OUT` );
  ...
  try {
    ...
    if ( items.length > 0 ) {
      ...
      if ( newItems === true ) {
        ...
        $.post(`/cart/add.js`, {items}, function ( r, textStatus, jqXHR ) {
...
function applyHTML( o ) {
  try {
    ...
    if ( typeof o.productSchema !== 'undefined' && o.productSchema ) {

      $( '.produtSchema' ).html( o.productSchema );

    }

    if ( typeof o.product_qty !== 'undefined' && o.product_qty == 0 ) {
    ...
    } else if ( typeof o.product_qty !== 'undefined' && o.product_qty > 0 ) {
