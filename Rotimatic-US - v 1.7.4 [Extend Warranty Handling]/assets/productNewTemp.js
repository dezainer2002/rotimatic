...
.on('click', '.btn_add2Cart.formBtn__add2Cart:not(.disabled)', function( e ) {
  try {
    ...
    if ( warrantyVariant != undefined ) {
      ...
      const getSelectedSKU    =   $( `.extend_WarrantyModal_ .extendedWarranty ._warranty_Item_.selected` ).attr( `sku` );
      $( `.extend_WarrantyModal_ .footerGrid .rightLink__` ).attr( `href`, `https://customers.extend.com/plan_details/${ getSelectedSKU }` );
...
async function isProductAvailable() {
  try {
    ...
    const selectorIs  =   $( `#shopify-section-product-variantTabs ._variants_ ._item_.selected` );
    const handle      =   selectorIs.attr( `handle` );
    const vid         =   selectorIs.attr( `vid` );

    const getResponse =   await getRequest( `/products/${ handle }?variant=${ vid }&view=getProductAvailable` );

    if ( getResponse.isAvailable != undefined ) {
      if ( getResponse.isAvailable == 'available' ) {
        $( `form.addToCartForm .btn_add2Cart` ).removeClass( `disabled` ).find( `> span` ).text( `ADD TO CART` );
      } else if ( getResponse.isAvailable == 'notAvailable' ) {
        $( `form.addToCartForm .btn_add2Cart` ).addClass( `disabled` ).find( `> span` ).text( `OUT OF STOCK` );
      }
    }
    ...
    console.log( `ERROR isProductAvailable`, err.message );
  }
}
