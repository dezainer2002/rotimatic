...
theme.Cart = (function() {
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    _setQuantityInputs: function($item, item, index) { 
     ...
     /* if(item.quantity > 4){
        alert("You can purchase max 4 qty.");
      return false;
      }else{*/
...
/*$.ajax({
        type: "GET",
        url: 'https://rotimatic.com/wp-json/wp/v2/roti-counter',
        async:true,
        dataType : 'json',         
...
setInterval(function rotimakercounter() {
  var rct = $('.rotimakecounter').data('count');
  var number = 1 + Math.floor(Math.random() * 10);
  var newrct= rct+number;
  $('.rotimakecounter').data('count', newrct);
   $('.rotimakecounter').html(addCommas(newrct));  
},
1000);

$('#rtp_check_submit').click(function() {    
  var machine_id = $('#rtp_warranty_serial').val();
  jQuery.post('https://betafbshop.premium-themes.co/rotimaticapp/check_waranty.php', {
    machine_id: machine_id},function(data){
    var old1yrwty = $('#1yearwarranty').attr("href");
    var old2yrwty = $('#2yearwarranty').attr("href");
    var new1yrwty = old1yrwty+'?attributes[Machine ID]='+machine_id;
    var new2yrwty = old2yrwty+'?attributes[Machine ID]='+machine_id;
    if (old1yrwty.indexOf("Machine ID") == -1){
    $('#1yearwarranty').attr("href", new1yrwty);
    $('#2yearwarranty').attr("href", new2yrwty);
  }
    var obj = JSON.parse(data); 
    if(obj.message=='not-eligible'){
      alert('You are not eligible to buy the extended warrenty.');
      $('.1-year-international-warranty').css('display','none');
      $('.2-year-international-warranty').css('display','none');
    }else if(obj.message=='eligible'){
      if(obj.year==1){
      alert('You are eligible to buy the extended warrenty.');  
      $('.1-year-international-warranty').css('display','block');
      $('.2-year-international-warranty').css('display','block');
      }
      if(obj.year==2){
      alert('You are eligible to buy the extended warrenty.');  
      $('.1-year-international-warranty').css('display','block');
      ...
      if(obj.year==3){
      alert('Your machine already has 3 year warranty.');  
      $('.1-year-international-warranty').css('display','none');
      $('.2-year-international-warranty').css('display','none');
      }
